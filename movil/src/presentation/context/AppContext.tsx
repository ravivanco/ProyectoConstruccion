...
interface AppContextValue { session: { authenticated: boolean; completed: boolean }; profile: PatientProfile; register(input: RegisterInput): Promise<void>; login(input: LoginInput): Promise<void>; updateProfile(changes: Partial<PatientProfile>): void; completeProfile(): Promise<void>; getPlanStatus(): Promise<NutritionPlanStatus | null>; getActivePlan(): Promise<ActiveNutritionPlan | null>; getPlanWeeks(planId: string): Promise<PlanWeek[]>; getDish(id: string): Promise<Dish | null>; getCalorieDashboard(): Promise<CalorieDashboard | null>; getExercises(category?: string): Promise<Exercise[]>; getRecommendedExercises(): Promise<Exercise[]>; getExerciseSchedule(planId: string, patientId?: string): Promise<AssignedExercise[]>; createWeightRecord(weightKg: number, notes?: string): Promise<WeightRecord>; getWeightRecords(): Promise<WeightRecord[]>; getDailyTracking(date: string): Promise<DailyTracking>; createMealLog(input: Omit<MealLog, 'id' | 'logDate'>): Promise<MealLog>; createAdditionalIntake(input: CreateAdditionalIntakeInput): Promise<void>; analyzeFoodImage(imageBase64: string): Promise<FoodImageAnalysis>; confirmAdditionalIntake(id: string): Promise<void>; discardAdditionalIntake(id: string): Promise<void>; reset(): void; }
const AppContext = createContext<AppContextValue | null>(null);

export function AppProvider({ children }: PropsWithChildren) {
  const services = useMemo(() => { const client = new HttpClient(); const auth = new AuthApiRepository(client); const storage = new SecureTokenStorage(); return { register: new RegisterPatient(auth, storage), login: new LoginPatient(auth, storage), complete: new CompleteProfile(new ProfileApiRepository(client)), plans: new NutritionPlanApiRepository(client), dishes: new DishApiRepository(client), calories: new CalorieControlApiRepository(client), exercises: new ExerciseApiRepository(client), tracking: new TrackingApiRepository(client) }; }, []);
