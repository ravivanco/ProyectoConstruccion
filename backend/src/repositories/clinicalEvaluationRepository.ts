...
function mapRow(row: Record<string, unknown>): ClinicalEvaluation {
  return {
    ...
    muscleMassPercentage: row.muscle_mass_percentage != null ? Number(row.muscle_mass_percentage) : undefined,
  ...
  const evaluationDate = input.evaluationDate ?? new Date().toISOString().slice(0, 10);
    ...
    const result = await pool.query(
      `INSERT INTO clinical_evaluations (
        ...
        body_fat_percentage, muscle_mass_percentage, waist_cm, notes, evaluation_date
      ...
      input.weightKg,
        ...
        input.muscleMassPercentage ?? null,
        input.waistCm ?? null,
        input.notes ?? null,
        evaluationDate,
      ],
...
export async function getClinicalEvaluationById(
  id: string,
): Promise<ClinicalEvaluation | null> {
  const result = await pool.query(
    `SELECT * FROM clinical_evaluations WHERE id = $1`,
    [id],
  );
